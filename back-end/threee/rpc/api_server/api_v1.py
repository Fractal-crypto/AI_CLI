...
from threee.constants import USERPATH_STRATEGIES
from threee.data.history import get_datahandler
from threee.exceptions import OperationalException
from threee.rpc import RPC
from threee.rpc.api_server.api_schemas import (AvailablePairs, Balances, BlacklistPayload,
...
from threee.rpc.api_server.deps import get_config, get_exchange, get_rpc, get_rpc_optional
...
def show_config(rpc: Optional[RPC] = Depends(get_rpc_optional), config=Depends(get_config)):
    ...
    if rpc:
        state = rpc._threee.state
        strategy_version = rpc._threee.strategy.version()
